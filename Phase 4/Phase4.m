...
while true
    for i = 1:27
        ff_data = ff_team_O(data10{i,1},data10{i,2},data10{i,3},10);
        X1 = optimize_car(@objective_function,ff_data,10);
    ...
    for i = 1:27
        ff_data = ff_team_O(data13{i,1},data13{i,2},data13{i,3},13);
        X1 = optimize_car(@objective_function,ff_data,13);
...
function ff_data = ff_team_O(pilot,chassis,pp,wheelsize)
    eval(char(pilot));
    eval(char(chassis));
    eval(char(pp));
...
function [ x1 ] = optimize_car(f, D, wheelsize) %#ok<*DEFNU>
...
%             fprintf('.')
    ...
    function [ x0 ] = opti_jump(x0,HG)
        max_x = x0;
        max_phi = objective_function(D,x0,wheelsize);
        for i = .1:.1:1
            if objective_function(D,coerce(x0+HG*i),wheelsize) > max_phi
                max_phi = objective_function(D,coerce(x0+HG*i),wheelsize);
...
function [phi] = objective_function(D,X,wheelsize)
    ...
    elseif wheelsize == 13
        D.car.wheel_front.c     = 50 - 46*(X(7)-250)/(1500-250);
        D.car.wheel_rear.c      = 50 - 46*(X(9)-250)/(1500-250);        
    end
    ...
    target_heave = 8 - 5.11 - 0.0362;
    target_pitch = 6 - 4.91 + 1.4966e-04;
    target_roll = 3 - 1.55 - 7.6204e-07;
